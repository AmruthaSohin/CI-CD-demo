...
class S3BucketTagger:
    ...
    def get_bucket_tags(self, bucket_name: str) -> dict[str, str]:
        try:
            ...
            if e.response["Error"]["Code"] == "NoSuchTagSet":
                self.logger.info(f"There s no existing tags found for buckets: {bucket_name}")
                return {}
    ...
    def apply_tags_to_bucket(
        ...
        try:
            ...
            if merge_existing:
                ...
                self.logger.info(
                    f"We are merging {len(filtered_tags)} new tags with {len(existing_tags)} existing tags"
                )
            else:
                final_tags = filtered_tags
                self.logger.info(f"We are replacing all tags with {len(filtered_tags)} new tags")
            ...
            self.logger.info(
                f"We have successfuly applied {len(final_tags)} tags to bucket: {bucket_name}"
            )
            return True
        
        except ClientError as e:
            self.logger.error(f"We failed to apply tags {bucket_name}: {e}")
            return False
    ...
    def remove_tags_from_bucket(
        ...
        try:
            ...
            self.logger.info(
                f"Removed successfully {removed_count} tags from bucket: {bucket_name}"
            )
            return True
        
        except ClientError as e:
            self.logger.error(f"Failed {bucket_name}: {e}")
            return False
